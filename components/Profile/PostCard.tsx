    ...
    return (
        <article className="border-b border-[#e5e5e5] bg-white px-4 py-5 sm:px-5">
            <div className="flex items-start gap-3">
                <PostCardHeader profile={profile} createdAt={displayPost.created_at} />
                    ...
                    {isEditing ? (
                        <PostEditForm
                            ...
                            onSaved={(updatedPost) => {
                                ...
                                setLikeCount(updatedPost.like_count ?? likeCount)
                                setCommentCount(updatedPost.comment_count ?? commentCount)
                                setIsEditing(false)
                            }}
                        />
                    ) : (
                        <PostCardContent post={displayPost} profile={profile} eagerMedia={eagerMedia} />
                    )}
            ...
            {!isEditing && hasOpenedComments && (
                <div className={isCommentsOpen ? "block pl-[52]" : "hidden"}>
                    <CommentsSection
                        postId={displayPost.id}
                        username={profile.username}
                        currentProfile={currentProfile}
                        onCommentCreated={() => setCommentCount((current) => current + 1)}
