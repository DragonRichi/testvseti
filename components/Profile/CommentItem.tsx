                        ...
                        <div className="mt-1 flex flex-wrap items-center gap-x-3 gap-y-1 px-2 text-xs text-main-gray">
                            <span className="whitespace-nowrap">
                                ...
                                {isEdited && (
                                    <>
                                        {" · "}
                                        <span title={`Изменено ${EDITED_DATE_FORMATTER.format(new Date(updatedAt))}`} className="cursor-default">
                                            изменено
                                        </span>
                                    </>
                                )}
                            ...
                            <button type="button" onClick={() => void handleLike()} className={`cursor-pointer whitespace-nowrap font-medium transition-colors ${isLiked ? "text-main-green" : "hover:text-main-green"}`}>
                                Нравится&nbsp;{likesCount > 0 && `(${likesCount})`}
                            </button>

                            <button type="button" onClick={() => setIsReplying((current) => !current)} className="cursor-pointer whitespace-nowrap font-medium transition-colors hover:text-main-green">
                                Ответить
                            </button>
                            {replyCount > 0 && (replyCount > 3 || !repliesLoaded) && (
                                <button type="button" onClick={toggleReplies} className="cursor-pointer whitespace-nowrap font-medium text-main-green transition-colors hover:underline">
                                    {isRepliesOpen ? "Скрыть ответы" : `Показать ответы (${replyCount})`}
                                </button>
                            )}
                        </div>
                    )}
            ...
            {isRepliesOpen && (
                <div className={depth === 0 ? "ml-6 mt-2 sm:ml-9" : "mt-2"}>
                    {isRepliesLoading && replies.length === 0 && (
                        <div className="flex items-center gap-2 py-2 text-xs text-main-gray">
                    ...
                    {repliesError && (
                        <div className="mt-2">
                            ...
                            <button type="button" onClick={retry} className="mt-2 flex cursor-pointer items-center gap-1.5 text-xs font-medium text-main-green hover:underline">
                    ...
                    {hasMore && remainingCount > 0 && (
                        <button type="button" disabled={isRepliesLoading} onClick={loadMore} className="mt-3 flex cursor-pointer items-center gap-2 text-xs font-medium text-main-green transition-colors hover:underline disabled:pointer-events-none disabled:opacity-60">
                            ...
                            <span>
                                Показать ещё ответов ({Math.min(REPLIES_PAGE_SIZE, remainingCount)})
                            </span>
                        </button>
                    )}
                </div>
