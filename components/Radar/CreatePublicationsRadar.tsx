...
function CreatePublicationsRadar({ currentUserId, suggestedProfiles }: Props) {
            ...
            <button key={profile.id} type="button" onClick={() => toggleProfile(profile)} className="flex w-full cursor-pointer items-center gap-3 rounded-xl p-3 text-left transition-colors hover:bg-green-50">
                ...
                <div className="min-w-0 flex-1">
                    <div className="flex min-w-0 items-center gap-2">
                        ...
                        {isCurrentUser && (
                            <span className="shrink-0 rounded-full bg-green-50 px-2 py-0.5 text-[10px] font-semibold text-main-green">
                                Вы
                            </span>
                        )}
                    </div>
    ...
    return (
        <div className="rounded-3xl border border-green-100 bg-white p-5 sm:p-7">
            <div>
                <h1 className="text-xl font-bold text-gray-900">Новый радар</h1>
                <p className="mt-1 text-sm text-main-gray">Создайте отдельную ленту из публикаций выбранных аккаунтов.</p>
            </div>

            <div className="mt-7">
                <label className="text-sm font-semibold text-gray-900">Название</label>
                <input type="text" value={name} onChange={(event) => setName(event.target.value)} maxLength={100} placeholder="Например, Интересные люди" className="mt-2 h-11 w-full rounded-xl border border-gray-200 bg-white px-4 text-sm outline-none transition-colors placeholder:text-gray-400 focus:border-main-green" />
            </div>

            <div className="mt-6">
                <label className="text-sm font-semibold text-gray-900">Сортировка публикаций</label>
            ...
            <div className="mt-6">
                <label className="text-sm font-semibold text-gray-900">Аккаунты</label>

                <div className="relative mt-2">
                    <Search className="absolute left-4 top-1/2 size-4 -translate-y-1/2 text-main-gray" />
                    <input type="text" value={search} onChange={(event) => setSearch(event.target.value)} placeholder="Найти пользователя" className="h-11 w-full rounded-xl border border-gray-200 bg-white pl-11 pr-10 text-sm outline-none transition-colors placeholder:text-gray-400 focus:border-main-green" />
                ...
                {search.trim().length >= 2 && (
                    <div className="mt-2 min-h-[74] overflow-hidden rounded-2xl border border-gray-100">
                        {searchResults.length > 0 ? (
                            searchResults.map(renderProfile)
                        ) : !isSearching ? (
                            <div className="flex min-h-[74] items-center justify-center px-4 text-center text-sm text-main-gray">Пользователи не найдены</div>
                        ) : null}
                    </div>
                )}
            </div>
            ...
            {suggestedProfiles.length > 0 && (
                <div className="mt-7">
                    <div>
                        ...
                        <div className="mt-1 text-xs text-main-gray">Ваши подписки</div>
                    </div>
            ...
            {error && (
                <div className="mt-5 rounded-xl bg-red-50 px-4 py-3 text-sm text-red-500">{error}</div>
            )}
